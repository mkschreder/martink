...
/* 
  ...
  Ported to Atmel AVR: Jiri Pittner <jiri@pittnerovi.com>

*/

//!!! assumes a random noise input on PD6
...
#define INT2CHARS(ptr, val) MACRO( *(uint32_t*)(ptr) = (val) )
...
{
  int i;
  for(x += ECIES_NUMWORDS, i = 0; i < ECIES_NUMWORDS; i++, s += 8)
    sprintf(s, "%08x", *--x);
}

/*missing in avr stdlib*/
/* NOT ANY MORE! :)
int strspn(const char *string, const char *charset)
{   
const char *p, *q;  
for (q = string; *q != '\0'; ++q) {
...
{
  char buf[4 * ECIES_NUMWORDS];
  int fh, r, s;
	//DDRD &= ~_BV(PD6); PORTD &= ~_BV(PD6);
  do {
		for(r=0; r<4 * ECIES_NUMWORDS; ++r)
		{
			uint8_t c=0;
			uint8_t d = rand() % 2; 
			//uint8_t d=PIND&(1<<PD6);
			while(d == (rand() % 2)) ++c; //(PIND&(1<<PD6))) ++c;
			buf[r]=c;
		}
...
{
  char buf[8 * ECIES_NUMWORDS + 1], *bufptr = buf + ECIES_NUMWORDS * 8 - (ECIES_DEGREE + 3) / 4;
  elem_t x, y;
  exp_t k;
  get_random_exponent(k);
  point_copy(x, y, base_x, base_y);
  point_mult(x, y, k);
  //printP(PSTR("Here is your new public/private key pair:\n"));
  //bitstr_to_hex(buf, x); printP(PSTR("Public key: ")); print(bufptr); printP(PSTR(":"));
  //bitstr_to_hex(buf, y); print(bufptr);
  //bitstr_to_hex(buf, k); printP(PSTR("\nPrivate key: ")); print(bufptr); printP(PSTR("\n"));
}*/
...
{
  int bufsize = (3 * (4 * ECIES_NUMWORDS) + 1 + 15) & ~15;
  char buf[bufsize];
		      ...
		      const char *Px, const char *Py)
{
  ...
  memcpy(msg + 8 * ECIES_NUMWORDS, text, len);
  XTEA_ctr_crypt(msg + 8 * ECIES_NUMWORDS, len, k1);
  XTEA_cbcmac(msg + 8 * ECIES_NUMWORDS + len, msg + 8 * ECIES_NUMWORDS, len, k2);
}
		     ...
		     const char *privkey)
{
  ...
  XTEA_cbcmac(mac, msg + 8 * ECIES_NUMWORDS, len, k2);
  if (memcmp(mac, msg + 8 * ECIES_NUMWORDS + len, 8))
    return -1;
  memcpy(text, msg + 8 * ECIES_NUMWORDS, len);
