...
// CHANNELS 
		...
		// CHANNELS DIALOG
		M2_EXTERN_XYLIST(el_mm_channel_values); 
		GUI_EXTERN_FRAME(el_mm_frame); 
		
		static char chsm_str[6][6]; 
		
		void fn_chsm_save_channel(); 
		void fn_chsm_load_channel(int ch){
			if(!(ch >= 0 && ch < 6)) return; 
			fn_chsm_save_channel(); 
			chsm_str[ch][0] = '-'; 
			chsm_str[ch][3] = '-'; 
			model.channel_id = ch; 
			memcpy(&model.channel, &model.ch[ch], sizeof(struct gui_channel)); 
		}
		void fn_chsm_save_channel(){
			int ch = model.channel_id; 
			for(int c = 0; c < 6; c++)
				sprintf(chsm_str[c], " C%d ", c); 
			if(!(ch >= 0 && ch < 6)) return; 
			memcpy(&model.ch[ch], &model.channel, sizeof(struct gui_channel)); 
		}
		void fn_chsm_btn_1(m2_el_fnarg_p fnarg) { 
			(void)fnarg; 
			fn_chsm_load_channel(0); 
		}
		void fn_chsm_btn_2(m2_el_fnarg_p fnarg) { 
			(void)fnarg; 
			fn_chsm_load_channel(1);
		}
		void fn_chsm_btn_3(m2_el_fnarg_p fnarg) { 
			(void)fnarg; 
			fn_chsm_load_channel(2);
		}
		void fn_chsm_btn_4(m2_el_fnarg_p fnarg) { 
			(void)fnarg; 
			fn_chsm_load_channel(3);
		}
		void fn_chsm_btn_5(m2_el_fnarg_p fnarg) { 
			(void)fnarg;  
			fn_chsm_load_channel(4);
		}
		void fn_chsm_btn_6(m2_el_fnarg_p fnarg) { 
			(void)fnarg; 
		...
		M2_BUTTON(el_chsm_btn_1, "", chsm_str[0], fn_chsm_btn_1); 
		M2_BUTTON(el_chsm_btn_2, "", chsm_str[1], fn_chsm_btn_2); 
		M2_BUTTON(el_chsm_btn_3, "", chsm_str[2], fn_chsm_btn_3); 
		M2_BUTTON(el_chsm_btn_4, "", chsm_str[3], fn_chsm_btn_4); 
		M2_BUTTON(el_chsm_btn_5, "", chsm_str[4], fn_chsm_btn_5); 
		M2_BUTTON(el_chsm_btn_6, "", chsm_str[5], fn_chsm_btn_6); 
		M2_LIST(list_chsm_top_btns) = {
			&el_chsm_btn_1, 
			&el_chsm_btn_2, 
			&el_chsm_btn_3, 
			&el_chsm_btn_4, 
		...
		M2_HLIST(el_chsm_top_btns_hlist, "", list_chsm_top_btns); 
		M2_ALIGN(el_chsm_top_btns, "-1|2w126h61x1y1", &el_chsm_top_btns_hlist); 
...
void fn_mm_btn_ch1(m2_el_fnarg_p fnarg) { 
	(void)fnarg; 
	memcpy(&model.channel, &model.ch[0], sizeof(model.channel)); 
}

M2_BUTTON(el_mm_ch1_label, NULL, "CH1", ); 
...
GUI_SWITCH(el_mm_swa_value, "w4h4", &model.sw[0].on); 
GUI_SWITCH(el_mm_swb_value, "w4h4", &model.sw[1].on); 

M2_LABEL(el_mm_swcd_label, NULL, "C/D");
GUI_SWITCH(el_mm_swc_value, "w4h4", &model.sw[2].on); 
GUI_SWITCH(el_mm_swd_value, "w4h4", &model.sw[3].on); 
M2_LIST(list_mm_sw) = {&el_mm_swab_label, &el_mm_swa_value, &el_mm_swb_value, &el_mm_swcd_label, &el_mm_swc_value, &el_mm_swd_value}; 
...
M2_VLIST(el_mm_chi, "x8y16l5w15", el_mm_chi_list); 
...
M2_VLIST(el_mm_swi, "x83y16l5w15", el_mm_swi_list); 
...
// horizontal left
GUI_CHANNEL(el_mm_ch3_sl, "x2y14w2h40", &chmin, &chmax, &chcenter, &model.ch[2].value); 
GUI_CHANNEL(el_mm_ch4_sl, "x6y12w40h2", &chmin, &chmax, &chcenter, &model.ch[3].value); 

GUI_CHANNEL(el_mm_ch2_sl, "x123y14w2h40", &chmin, &chmax, &chcenter, &model.ch[1].value); 
GUI_CHANNEL(el_mm_ch1_sl, "x81y12w40h2", &chmin, &chmax, &chcenter, &model.ch[0].value); 
