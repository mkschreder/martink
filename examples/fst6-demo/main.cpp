...
M2_ALIGN(top_el_tlsm2, "-1|1W64H64", &el_tlsm_hlist);
...
LIBK_THREAD(_demo_thread){
	...
	while(1){
		...
		channel_info.swa = (pressed & (1 << 10)) != 0; 
		channel_info.swb = (pressed & (1 << 11)) != 0; 
		channel_info.swc = (pressed & (1 << 12)) != 0; 
		channel_info.swd = (pressed & (1 << 13)) != 0; 
		
		snprintf(el_mm_swab_str, sizeof(el_mm_swab_str), " %s %s", (channel_info.swa)?"*":"-", (channel_info.swb)?"*":"-"); 
		snprintf(el_mm_swcd_str, sizeof(el_mm_swcd_str), " %s %s", (channel_info.swc)?"*":"-", (channel_info.swd)?"*":"-"); 
		
		for(int c = 0; c < 6; c++) {
			channel_info.ch[c] = (int)fst6_read_stick((fst6_stick_t)c); 
			
			if(!channel_info.calibrated){
				channel_info.ch[c] = 1000 + (channel_info.ch[c] >> 2); 
			
				if(channel_info.ch[c] < channel_info.range[c][0])
					channel_info.range[c][0] = channel_info.ch[c]; 
				if(channel_info.ch[c] > channel_info.range[c][1])
					channel_info.range[c][1] = channel_info.ch[c]; 
			} else {
				channel_info.ch[c] = map(1000 + (channel_info.ch[c] >> 2), 
					channel_info.range[c][0], channel_info.range[c][1], 1000, 2000); 
			}
		}
